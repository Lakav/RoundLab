...
bomb event type/player/timing fail deliberately. Utility effects are now locked
with compact per-round signatures for bucketed timing, duration, kind, team, and
position, so smoke/flash/HE/fire/decoy/bomb-plant regressions cannot hide behind
unchanged effect counts. Weapon fires are also locked with compact per-round
signatures for bucketed timing, shooter, normalized weapon, team, position, and
yaw, so wrong shooter/weapon/pose regressions cannot hide behind unchanged fire
counts. The full-quality test also
