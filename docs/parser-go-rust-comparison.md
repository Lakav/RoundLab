...
- Bomb event signatures still have timing/order differences on seven audited rounds: Ancient 9/14, Anubis 23, Cache 11, Inferno 2/11/20. The counts and event types now match, but exact timestamps are not fully identical to Go. The remaining differences are small `bomb_exploded` bucket offsets or synthesized `bomb_defuse_abort` timing. A targeted check of Rust frames around the defuse cases did not expose a reliable `useKey` signal, so earlier abort synthesis would be guesswork for now.
