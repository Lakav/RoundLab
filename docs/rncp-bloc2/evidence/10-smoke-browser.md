...
| déploiement candidate courante | `07fb59a` | checkout prouvé, HTTP 200, chunk `3gzjaewd3cdr_.js` | PASS distant le 21/07 |
| rollback candidate courante | `07fb59a` → `2e51eaf` | checkout prouvé, HTTP 200, chunk remplacé par `0g-6joqzacvgb.js` | PASS distant le 21/07 |
| restauration candidate courante | `2e51eaf` → `07fb59a` | checkout prouvé, HTTP 200, chunk `3gzjaewd3cdr_.js` rétabli | PASS distant le 21/07 |
...
La séquence v0.1.39 est détaillée dans `deployment-runs-2026-07-20.md`. Les deux séquences successives de la candidate v0.1.40, dont le rollback et la restauration de la révision courante `07fb59a`, sont détaillées dans `deployment-runs-2026-07-21.md`.
