...
impl<'a> SecondPassParser<'a> {
    ...
    fn collect_c4(&mut self) {
        let Some(c4_entity_id) = self.c4_entity_id else {
            return;
        };
        let owner_entity_id = self
            .prop_controller
            .special_ids
            .h_owner_entity
            .and_then(|id| self.get_prop_from_ent(&id, &c4_entity_id).ok())
            .and_then(|value| match value {
                Variant::U32(handle) => Some((handle & 0x7FF) as i32),
                _ => None,
            });
        let x = self.collect_cell_coordinate_grenade(CoordinateAxis::X, &c4_entity_id).ok();
        let y = self.collect_cell_coordinate_grenade(CoordinateAxis::Y, &c4_entity_id).ok();
        let z = self.collect_cell_coordinate_grenade(CoordinateAxis::Z, &c4_entity_id).ok();
        self.c4_records.push(C4Record {
            x: variant_to_f32(x),
            y: variant_to_f32(y),
            z: variant_to_f32(z),
            tick: Some(self.tick),
            owner_entity_id,
            entity_id: Some(c4_entity_id),
        });
    }
...
fn variant_to_f32(value: Option<Variant>) -> Option<f32> {
    match value {
        ...
        Some(Variant::I32(v)) => Some(v as f32),
        Some(Variant::U32(v)) => Some(v as f32),
        _ => None,
    }
}
