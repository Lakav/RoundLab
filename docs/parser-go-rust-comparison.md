...
- Remaining projectile track tolerance deltas are limited but not gone: 3 rounds differ, with 0 missing Rust tracks, 0 extra Rust tracks, and 6 tolerant mismatches. The remaining cases are Ancient round 16 smoke duration, Anubis round 34 smoke duration, and Inferno round 17 repeated HE identity/end timing.
...
5. Investigate the remaining 3 projectile tolerance rounds without overmerging same-player repeated utility tracks; any fix must keep Rust at zero duplicate projectiles, zero non-monotonic frames, zero track breaks, and zero teleport-like jumps.
